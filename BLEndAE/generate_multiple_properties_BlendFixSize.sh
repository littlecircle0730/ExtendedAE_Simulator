...
generate_properties() {
    ...
    while IFS= read -r line; do
        ...
        filename="./BLEndAE/AE_FixSize_${numNodes}n_${discProb}_${latency}s_blend.properties"
        cat > "$filename" <<-EOF
...
for discProb in "${discProbList[@]}"; do
    for latency in "${latencyList[@]}"; do
        generate_properties "../AE_blendFixSize_${discProb}_${latency}s_log.txt" --disc_prob "${discProb}"
    done
done