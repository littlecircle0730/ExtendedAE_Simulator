...
for discProb in "${discProbList[@]}"; do
    for latency in "${latencyList[@]}"; do
        for param in "${params[@]}"; do
            java BLEDiscSimulator "./BLEndAE/AE_blend_${param}n_${discProb}_${latency}s.properties" \
                                "./output/AE_blend_${param}n_${discProb}_${latency}s.log" \
                                "./output_dc/AE_blend_${param}n_${discProb}_${latency}s.dc" \
                                1000
        done
    done
done