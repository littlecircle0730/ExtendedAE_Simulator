...
generate_properties() {
    ...
    while IFS= read -r line; do
        ...
        elif echo "$line" | grep -q "The best W is"; then
            # Extract W value
            W=$(echo "$line" | grep -oE "\d+" | tail -1)
        elif echo "$line" | grep -q "The best n is"; then
            # Extract n value and handle -1 case
            n=$(echo "$line" | awk -F'is ' '{print $2}')

            # Set the filename using the extracted n value and the provided discProb value
            filename="./BLEndAE/ExtendedAE_nihao_${numNodes}n_${discProb}_${latency}s_${n_chunk}chunk.properties"
...
# protocol (options: blend, searchlight, nihao, extendednihao)
protocol = extendednihao
...
for n_chunk in "${chunkList[@]}"; do
    for discProb in "${discProbList[@]}"; do
        for latency in "${latencyList[@]}"; do
            generate_properties "../ExtendedAE_nihao_${discProb}_${latency}s_${n_chunk}chunk_log.txt" --disc_prob "${discProb}"
        done
    done
done