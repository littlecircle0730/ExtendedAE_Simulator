...
# Function to generate properties for "ExtendedAE"
generate_properties() {
    discProb=""
    ...
    while IFS= read -r line; do
        ...
        filename="./BLEndAE/ExtendedAE_${numNodes}n_${discProb}_${latency}s_${n_chunk}chunk.properties"
        cat > "$filename" <<-EOF
...
for n_chunk in "${chunkList[@]}"; do
    for discProb in "${discProbList[@]}"; do
        for latency in "${latencyList[@]}"; do
            generate_properties "../ExtendedAE_blend_${discProb}_${latency}s_${n_chunk}chunk_log.txt" --disc_prob "${discProb}"
        done
    done
done