...
public class NihaoAESchedule extends BLESchedule {
	...
	void createSchedule() {
		if (schedule == null) {
			...
			while (windowStartTime < simulationTime) {
				...
				// 	double epochEndTime = Math.min(windowEndTime, epochStartTime + E);
				while (epochStartTime < windowStartTime + W) { // the rest of T does not adv or listen
					double epochEndTime = Math.min(windowStartTime + W, epochStartTime + W); 
	...
	private void createOneEpoch(int channel, double startTime, double extraEpoch, int chunkNumber, double epochEndTime) {
		...
		while (time + totalBeaconTime < epochEndTime) {
			...
			schedule.add(startAdvertising);
			BLEExtendedAdvertiseEndEvent endAdvertising = new BLEExtendedAdvertiseEndEvent(nodeID, time+totalBeaconTime, -1, chunkNumber);
			schedule.add(endAdvertising);
